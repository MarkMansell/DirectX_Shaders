...
bool ShadowShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	result = InitializeShader(device, hwnd, L"ShadowMapping.fx", L"ShadowMapping.fx");
	if (!result)
	{
		return false;
	}
...
bool ShadowShaderClass::Render(ID3D11DeviceContext* deviceContext, int indexCount, const XMMATRIX& worldMatrix, const XMMATRIX& viewMatrix,
...
bool ShadowShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	result = CompileShaderFromFile(L"ShadowMapping.fx", "VS", "vs_4_0", &vertexShaderBuffer);
	...
	result = CompileShaderFromFile(L"ShadowMapping.fx", "PS", "ps_4_0", &pixelShaderBuffer);
...
HRESULT ShadowShaderClass::CompileShaderFromFile(WCHAR* szFileName, LPCSTR szEntryPoint, LPCSTR szShaderModel, ID3DBlob** ppBlobOut)
{
	HRESULT hr = S_OK;
...
bool ShadowShaderClass::SetShaderParameters(ID3D11DeviceContext* deviceContext, const XMMATRIX& worldMatrix, const XMMATRIX& viewMatrix,
	const XMMATRIX& projectionMatrix, const XMMATRIX& lightViewMatrix, const XMMATRIX& lightProjectionMatrix,
	ID3D11ShaderResourceView* texture, ID3D11ShaderResourceView* depthMapTexture, XMFLOAT3 lightPosition,
	XMFLOAT4 ambientColor, XMFLOAT4 diffuseColor)
{
	...
	XMMATRIX lightview = lightViewMatrix;
	XMMATRIX lightprojection = lightProjectionMatrix;
