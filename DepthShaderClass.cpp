...
bool DepthShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	result = InitializeShader(device, hwnd, L"DepthMapping.fx", L"DepthMapping.fx");
	if (!result)
	{
		return false;
	}
...
bool DepthShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	result = CompileShaderFromFile(L"DepthMapping.fx", "DepthVertexShader", "vs_4_0", &vertexShaderBuffer);
	if (FAILED(result))
	{
	...
	result = CompileShaderFromFile(L"DepthMapping.fx", "DepthPixelShader", "ps_4_0", &pixelShaderBuffer);
	if (FAILED(result))
	{
		// If the shader failed to compile it should have writen something to the error message.
		if (errorMessage)
...
HRESULT DepthShaderClass::CompileShaderFromFile(WCHAR* szFileName, LPCSTR szEntryPoint, LPCSTR szShaderModel, ID3DBlob** ppBlobOut)
{
	HRESULT hr = S_OK;
...
bool DepthShaderClass::Render(ID3D11DeviceContext* deviceContext, int indexCount, const XMMATRIX& worldMatrix, const XMMATRIX& viewMatrix,
...
bool DepthShaderClass::SetShaderParameters(ID3D11DeviceContext* deviceContext, const XMMATRIX& worldMatrix, const XMMATRIX& viewMatrix, const XMMATRIX& projectionMatrix)
{
	HRESULT result;
